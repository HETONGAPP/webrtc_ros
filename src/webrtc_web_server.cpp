...
public:
  WebrtcWebServerImpl(rclcpp::Node::SharedPtr nh, int port,
    ...
    std::vector<async_web_server_cpp::HttpHeader> file_headers;
    std::vector<async_web_server_cpp::HttpHeader> ip_headers;
    std::string file_path = "/home/khadas/rtabmap_data/database.db";
    std::string ip_path = "/home/khadas/settings/networks/ip.txt";
    //auto now_ms = std::chrono::time_point_cast<std::chrono::milliseconds>(std::chrono::system_clock::now());
    //auto timestamp = std::to_string(now_ms.time_since_epoch().count());
    //std::string filename = "database_" + timestamp + ".db";
    ...
    file_headers.push_back(async_web_server_cpp::HttpHeader(
        "Content-Disposition", "attachment; filename=database.db"));
    
    ip_headers.push_back(async_web_server_cpp::HttpHeader(
        "Content-Disposition", "attachment; filename=ip.txt"));
        
        
    handler_group_.addHandlerForPath(
        "/download", async_web_server_cpp::HttpReply::from_file(
                         async_web_server_cpp::HttpReply::ok,
                         "application/octet-stream", file_path, file_headers));
                         
    handler_group_.addHandlerForPath(
        "/remote_ip", async_web_server_cpp::HttpReply::from_file(
                         async_web_server_cpp::HttpReply::ok,
                         "application/octet-stream", ip_path, ip_headers));

    handler_group_.addHandlerForPath(
        "/index", async_web_server_cpp::HttpReply::from_file(
